...
# ── Peg geometry — offsets in EE (pinch_site) local frame ────────────────────
# Peg is assumed rigidly attached to the gripper. When the EE rotates, the peg
# rotates with it. World-frame peg site positions are:
#   peg_site_world = ee_pos + ee_rot @ PEG_OFFSET_IN_EE_*
# From peg.xml: cylinder_start at local pos="0 0 -0.02", cylinder_end at "0 0 -0.07"
# relative to the peg body origin (= pinch_site on reset).
# Adjust if real-world peg mounting differs from sim.
PEG_CYLINDER_START_IN_EE = np.array([0.0, 0.0, -0.02])   # in EE local frame [m]
PEG_CYLINDER_END_IN_EE   = np.array([0.0, 0.0, -0.07])   # in EE local frame [m]

# ── Hole position and site offsets — fixed, matches sim reset_hole_position ──
...
def build_obs(q, ee_pos, ee_rot, home_rad, hole_top, hole_bottom, last_action) -> np.ndarray:
    ...
    """
    # Peg site world positions — rotate EE-frame offsets by current EE orientation
    peg_start_w = ee_pos + ee_rot @ PEG_CYLINDER_START_IN_EE   # (3,)
    peg_end_w   = ee_pos + ee_rot @ PEG_CYLINDER_END_IN_EE     # (3,)

    joint_pos_rel = q - home_rad                                   # (7,)
    peg_to_hole   = np.concatenate([hole_top    - peg_start_w,    # (3,)
                                    hole_bottom - peg_end_w])     # (3,) → (6,)
    ee_to_peg_obs = peg_start_w - ee_pos                          # (3,) = ee_rot @ offset
...
def policy_process_fn(checkpoint_path, device_str, home_rad_arr,
    ...
    hole_top    = HOLE_TOP_WORLD.copy()
    hole_bottom = HOLE_BOTTOM_WORLD.copy()
        ...
        while True:
            ...
            if cb_peg.value:
                peg_start_marker_sim.position  = tuple(float(v) for v in
                    ee_pos_sim  + ee_rot_sim  @ PEG_CYLINDER_START_IN_EE)
                peg_end_marker_sim.position    = tuple(float(v) for v in
                    ee_pos_sim  + ee_rot_sim  @ PEG_CYLINDER_END_IN_EE)
                peg_start_marker_real.position = tuple(float(v) for v in
                    ee_pos_real + ee_rot_real @ PEG_CYLINDER_START_IN_EE)
                peg_end_marker_real.position   = tuple(float(v) for v in
                    ee_pos_real + ee_rot_real @ PEG_CYLINDER_END_IN_EE)
            ...
            peg_start_sim = ee_pos_sim + ee_rot_sim @ PEG_CYLINDER_START_IN_EE
