#!/bin/bash
#SBATCH --job-name=open_door_osc
#SBATCH --partition=1gpu
#SBATCH --nodes=1
#SBATCH --nodelist=dgx1
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --output=/ihub/homedirs/svs_ald/sudhir/manipulation/logs/open_door_osc.log
#SBATCH --qos=1gpu
...
env MUJOCO_GL=egl \
    ...
    uv run train Mjlab-Open-Door-Osc-Kinova \
    --env.scene.num-envs 1024 \
    --agent.max-iterations 5_000 \
    --agent.wandb-project mjlab-kinova-tasks-osc \
    --agent.experiment-name open_door_osc \
    --video True \
    --video-length 100 \
    --video-interval 100
