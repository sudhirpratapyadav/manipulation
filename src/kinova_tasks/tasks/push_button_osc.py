"""Kinova Gen3 push-button task with Operational Space Control (OSC, relative mode).

Task: Press the button down (vertical slide joint) to a randomly sampled target
depth. The policy controls the 6D EE pose (OSC relative mode) plus a 1D gripper
open/close.
...
from kinova_tasks.assets.objects.articulated.button.button_constants import get_button_cfg
...
# Home joint positions: matches open_door / open_drawer (joint_1 = 0°, arm
# facing forward along +x in robot-base frame).
_HOME_JOINT_POS = (
    0.0,            # joint_1   0°
...
# Button spawn range (local frame) — button_base mocap body position.
# z range is ±10 cm around the robot base (z=0): the base plate top sits at
# button_base.z, and collisions with the ground are disabled, so the button
# can spawn near (and even below) the floor.
_BUTTON_POS_LO = (0.45, -0.25, 0.0)
_BUTTON_POS_HI = (0.85,  0.25, 0.15)

# Button initial slide range at reset (meters): 0 = unpressed, -0.05 = fully pressed.
# Start fully unpressed.
_BUTTON_INIT_SLIDE_LO = 0.0
_BUTTON_INIT_SLIDE_HI = 0.0

# Button goal slide range (meters): target is "pressed down" near the bottom of
# the joint range. button_slide joint range: [-0.05, 0] (negative = pressed in).
_BUTTON_GOAL_LO = -0.05  # fully pressed
_BUTTON_GOAL_HI = -0.04  # almost fully pressed

# Button handle geometry (from button.xml):
#   object_site is at pos="0 0 0.055" in the handle body frame.
#   handle body slides along z-axis (axis="0 0 1") within button_base.
#   At slide joint q=0 (unpressed) handle bottom sits 5 cm above base plate top;
#   at q=-0.05 (fully pressed) handle bottom touches the base plate top.
#   At slide joint value q, object_site world pos =
#       button_base_pos_w + (0, 0, 0.055 + q)
_HANDLE_OFFSET_X = 0.0
_HANDLE_OFFSET_Y = 0.0
_HANDLE_OFFSET_Z = 0.055


# ---------------------------------------------------------------------------
# Gripper action term (identical to pick_cube_osc / open_door_osc)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Button goal command term
# ---------------------------------------------------------------------------


class ButtonGoalCommand(CommandTerm):
    """Samples and maintains a target button slide position for the push-button task."""

    cfg: ButtonGoalCommandCfg

    def __init__(self, cfg: ButtonGoalCommandCfg, env: ManagerBasedRlEnv):
        super().__init__(cfg, env)
        self.goal_slide = torch.zeros(self.num_envs, 1, device=self.device)

    @property
    def command(self) -> torch.Tensor:
        """Target button slide position (m), shape (num_envs, 1).

        Negative values mean pressed in. Range: [_BUTTON_GOAL_LO, _BUTTON_GOAL_HI].
        """
    ...
    def _debug_vis_impl(self, visualizer: DebugVisualizer) -> None:
        ...
        button: Entity = self._env.scene["button"]

        # Spawn range bounding box edges (blue, 3D wireframe)
        x_lo, y_lo, z_lo = _BUTTON_POS_LO
        x_hi, y_hi, z_hi = _BUTTON_POS_HI
        corners = np.array([
        ...
        for i in env_indices:
            ...
            for idx, (a, b) in enumerate(spawn_edges):
                visualizer.add_cylinder(
                    ...
                    label=f"button_spawn_edge_{i}_{idx}",
                )

            # Goal handle position (orange sphere): linear FK along z
            button_pos = button.data.root_link_pos_w[i].cpu().numpy()
            q_goal = float(self.goal_slide[i, 0].item())
            target_handle_w = button_pos + np.array(
                [_HANDLE_OFFSET_X, _HANDLE_OFFSET_Y, _HANDLE_OFFSET_Z + q_goal],
                dtype=np.float32,
            )
            visualizer.add_sphere(
                center=target_handle_w, radius=0.03,
                color=(1.0, 0.5, 0.0, 0.4), label=f"button_goal_{i}",
            )
            visualizer.add_sphere(
                center=target_handle_w, radius=0.008,
                color=(1.0, 0.6, 0.0, 0.9), label=f"button_goal_dot_{i}",
            )


@dataclass(kw_only=True)
class ButtonGoalCommandCfg(CommandTermCfg):
    """Configuration for the button goal command term."""

    slide_lo: float = _BUTTON_GOAL_LO
    slide_hi: float = _BUTTON_GOAL_HI

    def build(self, env: ManagerBasedRlEnv) -> ButtonGoalCommand:
        return ButtonGoalCommand(self, env)


def _get_button_goal_command(env: ManagerBasedRlEnv, command_name: str = "button_goal") -> ButtonGoalCommand:
    term = env.command_manager.get_term(command_name)
    assert isinstance(term, ButtonGoalCommand)
    return term
...
def reset_button(
    env: ManagerBasedRlEnv,
    env_ids: torch.Tensor | None,
    button_entity_name: str = "button",
    x_range: tuple[float, float] = (_BUTTON_POS_LO[0], _BUTTON_POS_HI[0]),
    y_range: tuple[float, float] = (_BUTTON_POS_LO[1], _BUTTON_POS_HI[1]),
    z_range: tuple[float, float] = (_BUTTON_POS_LO[2], _BUTTON_POS_HI[2]),
    init_slide_range: tuple[float, float] = (_BUTTON_INIT_SLIDE_LO, _BUTTON_INIT_SLIDE_HI),
) -> None:
    """Reset button base position (xyz uniform) and slide joint to initial position."""
    if env_ids is None:
        env_ids = torch.arange(env.num_envs, device=env.device, dtype=torch.int)

    button: Entity = env.scene[button_entity_name]
    n = len(env_ids)
    ...
    button.write_mocap_pose_to_sim(pose, env_ids=env_ids)
    ...
    button.write_joint_state_to_sim(jpos, jvel, env_ids=env_ids)


# ---------------------------------------------------------------------------
# Observation functions
# Mirrors open_drawer_osc: joint_vel, ee_pose, gripper_state,
#   ee_to_object, object_pos, object_to_goal, goal_pos, actions
# The button handle (object_site) is the manipulated "object".
# ---------------------------------------------------------------------------
...
def ee_to_object(
    ...
    """Vector from EE (pinch_site) to button handle (object_site) in world frame (3D).
    ...
    button: Entity = env.scene[button_entity_name]
    handle_ids = button.find_sites("object_site")[0]
    handle_pos_w = button.data.site_pos_w[:, handle_ids].squeeze(1)
...
def object_pos(
    ...
    button: Entity = env.scene[button_entity_name]
    handle_ids = button.find_sites("object_site")[0]
    return button.data.site_pos_w[:, handle_ids].squeeze(1) - env.scene.env_origins


def _goal_handle_pos_w(
    env: ManagerBasedRlEnv,
    button_entity_name: str = "button",
    command_name: str = "button_goal",
) -> torch.Tensor:
    """Compute goal handle position in world frame via linear FK (N, 3).

    Button slide joint moves along z-axis in button_base frame (identity orientation).
    At goal slide value q_goal, object_site world pos is:
        button_base_pos_w + (0, 0, _HANDLE_OFFSET_Z + q_goal)
    """
    button: Entity = env.scene[button_entity_name]
    button_base_w = button.data.root_link_pos_w  # (N, 3)

    q_goal = _get_button_goal_command(env, command_name).goal_slide  # (N, 1)

    tx = torch.full_like(q_goal, _HANDLE_OFFSET_X)         # (N, 1)
    ty = torch.full_like(q_goal, _HANDLE_OFFSET_Y)         # (N, 1)
    tz = q_goal + _HANDLE_OFFSET_Z                          # (N, 1)

    return button_base_w + torch.cat([tx, ty, tz], dim=-1)  # (N, 3)
...
def goal_pos(
    ...
    return _goal_handle_pos_w(env, button_entity_name, command_name) - env.scene.env_origins
...
def object_to_goal(
    ...
    handle_ids = button.find_sites("object_site")[0]
    handle_pos_w = button.data.site_pos_w[:, handle_ids].squeeze(1)
    target_w = _goal_handle_pos_w(env, button_entity_name, command_name)
    return target_w - handle_pos_w


# ---------------------------------------------------------------------------
# Reward functions (mirrors open_drawer_osc)
# ---------------------------------------------------------------------------
...
def ee_to_object_reward(
    ...
    """Gaussian reward for EE proximity to button handle (reach phase)."""
    robot: Entity = env.scene[asset_cfg.name]
    ee_pos = robot.data.site_pos_w[:, asset_cfg.site_ids].squeeze(1)
    button: Entity = env.scene[button_entity_name]
    handle_ids = button.find_sites("object_site")[0]
...
class object_at_goal_reward:
    """Gaussian reward for handle proximity to goal position (press phase).

    Debug vis draws:
      - Green sphere at current handle position
      - EE coordinate frame at pinch_site
    (Goal sphere is drawn by ButtonGoalCommand._debug_vis_impl)
    """

    def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv):
        self._env = env
        self._button_entity_name: str = cfg.params.get("button_entity_name", "button")
        self._command_name: str = cfg.params.get("command_name", "button_goal")
    ...
    def debug_vis(self, visualizer: DebugVisualizer) -> None:
        ...
        button: Entity = env.scene[self._button_entity_name]
        robot: Entity = env.scene["robot"]
        handle_ids = button.find_sites("object_site")[0]

        for i in env_indices:
            # Current handle: green sphere
            handle_pos_np = button.data.site_pos_w[i, handle_ids].squeeze(0).cpu().numpy()
            visualizer.add_sphere(
...
def ee_to_object_error(
    ...
    """Euclidean distance from EE to button handle."""
    robot: Entity = env.scene[asset_cfg.name]
    ee_pos = robot.data.site_pos_w[:, asset_cfg.site_ids].squeeze(1)
    button: Entity = env.scene[button_entity_name]
    handle_ids = button.find_sites("object_site")[0]
...
def button_out_of_range(
    env: ManagerBasedRlEnv,
    button_entity_name: str = "button",
) -> torch.Tensor:
    """Terminate if the button slide joint goes far outside its valid range [-0.05, 0]."""
    button: Entity = env.scene[button_entity_name]
    slide = button.data.joint_pos.squeeze(-1)
    return (slide > 0.02) | (slide < -0.10)
...
def kinova_push_button_osc_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    """Kinova Gen3 push-button task with OSC (relative mode) + gripper control.

    Button starts unpressed. The policy must press the button down by reaching
    the handle and pushing it to a randomly sampled target slide depth in
    [-0.05 m, -0.04 m] (near-fully-pressed).
    ...
    # --- Observations ---
    # Mirrors open_drawer_osc. Handle = "object"; goal = FK-computed target handle pos.
    ...
    actor_terms = {
        ...
        "object_pos": ObservationTermCfg(
            func=object_pos,
            params={"button_entity_name": "button"},
            noise=Unoise(n_min=-0.01, n_max=0.01),
        ),
        "object_to_goal": ObservationTermCfg(
            func=object_to_goal,
            params={"button_entity_name": "button", "command_name": "button_goal"},
            noise=Unoise(n_min=-0.01, n_max=0.01),
        ),
    ...
    events = {
        ...
        "reset_button": EventTermCfg(
            func=reset_button,
            mode="reset",
            params={
                "button_entity_name": "button",
                "x_range": (_BUTTON_POS_LO[0], _BUTTON_POS_HI[0]),
                "y_range": (_BUTTON_POS_LO[1], _BUTTON_POS_HI[1]),
                "z_range": (_BUTTON_POS_LO[2], _BUTTON_POS_HI[2]),
                "init_slide_range": (_BUTTON_INIT_SLIDE_LO, _BUTTON_INIT_SLIDE_HI),
            },
        ),
    ...
    rewards = {
        ...
        # Phase 2: Handle reaches goal position (Gaussian, std=0.05 m).
        # Button stroke is only 5 cm so std must be small to give a useful
        # gradient near the pressed state.
        "move_to_goal": RewardTermCfg(
            func=object_at_goal_reward,
            weight=1.0,
            params={"std": 0.05},
        ),
        # Tight placement bonus (Gaussian, std=0.02 m ~2 cm)
    ...
    cfg = ManagerBasedRlEnvCfg(
        ...
        actions=actions,
        commands={
            "button_goal": ButtonGoalCommandCfg(
...
def kinova_push_button_osc_ppo_cfg() -> RslRlOnPolicyRunnerCfg:
    """PPO config for push-button OSC task."""
    return kinova_ppo_runner_cfg(experiment_name="kinova_push_button_osc")
